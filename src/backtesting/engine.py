...
class BacktestEngine:
    ...
    def _load_config(self, overrides: Optional[Dict[str, Any]] = None) -> None:
        ...
        config_dict = {
            'start_date': get_config_value('backtesting.start_date'),
            'end_date': get_config_value('backtesting.end_date'),
            'initial_balance': get_config_value('backtesting.initial_balance'),
            'tick_file': get_config_value('backtesting.data_files.tick_file'),
            'l2_file': get_config_value('backtesting.data_files.l2_file'),
            'model_path': get_config_value('backtesting.model_path'),
            'commission_rate': get_config_value('backtesting.execution.commission_rate'),
            'min_prediction_threshold': get_config_value('backtesting.trading.min_prediction_threshold'),
            'lookback_window': get_config_value('backtesting.trading.lookback_window'),
            'symbol': get_config_value('backtesting.symbol', 'XAUUSD')
        }
    ...
    def _update_market_state(self, tick_data: pd.Series) -> None:
        """Update exchange simulator with current market data."""
        bid = tick_data.get('bid', tick_data.get('bid_price_1', 0.0))
        ask = tick_data.get('ask', tick_data.get('ask_price_1', 0.0))
    ...
    def _update_feature_buffer(self, tick_data: pd.Series) -> None:
        ...
        # Keep only required lookback window
        if len(self.state.feature_buffer) > self.config.lookback_window:
    ...
    def _get_prediction(self, features_df: pd.DataFrame) -> Optional[float]:
        """Get model prediction for current market conditions."""
        try:
            prediction = self.model_predictor.predict(features_df)
            return prediction[0] if len(prediction) > 0 else None
        except Exception as e:
            self.logger.warning("Prediction failed", error=str(e))
            return None
    
    def _process_trading_signal(self, prediction: Optional[float], tick_data: pd.Series) -> None:
        """Process trading signal with comprehensive risk management."""
        if prediction is None:
            return
        
        # Check if prediction meets minimum threshold
        if abs(prediction) < self.config.min_prediction_threshold:
            return
        
        # Get current market prices
        bid = float(tick_data.get('bid', tick_data.get('bid_price_1', 0.0)))
        ask = float(tick_data.get('ask', tick_data.get('ask_price_1', 0.0)))
        ...
        position_size_result = self._calculate_position_size(prediction, tick_data)
        ...
        self._submit_trade_order(side, position_size_result.size, mid_price)
    
    def _calculate_position_size(self, prediction: float, tick_data: pd.Series) -> Any:
        ...
        current_price = (tick_data.get('bid', 0) + tick_data.get('ask', 0)) / 2.0
        
        # Use historical performance estimates (these would be calculated from training data)
        win_probability = 0.52  # Example values - in production, use actual statistics
        avg_win = 50.0
        avg_loss = 40.0
    ...
    def _submit_trade_order(self, side: str, size: float, price: float) -> None:
        """Submit trade order to exchange simulator."""
        try:
            order_result = self.exchange_simulator.submit_order(
                symbol=self.config.symbol,
                side=side,
                quantity=size,
                order_type='MARKET',
            ...
            if order_result:
                ...
                self.risk_manager.record_trade(self.config.symbol, size, side)
    ...
    def _calculate_fill_pnl(self, fill) -> float:
        """Calculate PnL for a filled order (simplified for single-position model)."""
        # This is a simplified PnL calculation
        # In practice, you'd track positions and calculate unrealized/realized PnL
        return 0.0  # Placeholder - implement based on your position tracking needs
    ...
    def _update_performance_tracking(self) -> None:
        ...
        # Update max drawdown
        peak_balance = self.config.initial_balance + max(0, self.state.total_pnl)
    ...
    def _generate_results(self) -> Dict[str, Any]:
        ...
        returns_series = pd.Series([p['pnl'] for p in self.performance_history])
        sharpe_ratio = returns_series.mean() / max(returns_series.std(), 0.001) * np.sqrt(252 * 24 * 60)  # Annualized
    ...
    def save_results(self, results: Dict[str, Any], output_dir: Optional[Path] = None) -> Path:
        ...
        # Generate report
        report_path = generate_backtest_report(results, output_dir)
        
        self.logger.info("Backtest results saved", report_path=str(report_path))
        return report_path