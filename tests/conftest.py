...
def sample_price_series(sample_data_df: pd.DataFrame) -> pd.Series:
    ...
    Args:
        sample_data_df: The main test dataset fixture.
        
    Returns:
        pd.Series: Close prices with proper datetime index.
    """
...
def market_data_small() -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Minimal test dataset with 20 rows.
    """
    periods = 20
    index = pd.date_range(
    ...
    # Simple linear price progression for predictable test results
    base_price = 2000.0
    price_increment = 0.1
